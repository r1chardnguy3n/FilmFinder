...
namespace FilmFinderWebApp.ViewComponents
{
    public class UpComingViewComponent : ViewComponent 
    {
        public async Task<IViewComponentResult> InvokeAsync()
        {
            ...
            SearchContainer<SearchMovie> upComing = await client.GetMovieUpcomingListAsync();

            var getUpComing = (from result in upComing.Results.Take(15)
                                 select new
                                 {
                                     Title = result.Title,
